...
class SportradarAPI {
  ...
  private async request<T>(endpoint: string, params?: Record<string, any>): Promise<T> {
    try {
      ...
      throw new APIError(`Hálózati hiba: ${error.message}`, 500, 'NETWORK_ERROR');
    }
  }
  ...
  getLiveMatches(): Promise<LiveMatchesResponse> {
    return this.request('/soccer/trial/v4/en/matches/live.json');
  }
  ...
  getMatchSummary(matchId: string): Promise<MatchSummaryResponse> {
    ...
    return this.request(`/soccer/trial/v4/en/matches/${numericId}/summary.json`);
  }
  ...
  getScheduledMatches(dateISO: string): Promise<SeasonScheduleResponse> {
    // dateISO: YYYY-MM-DD
    return this.request(`/soccer/trial/v4/en/schedules/${dateISO}/schedule.json`);
  }
